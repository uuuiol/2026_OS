...
/*
 * ============================================================
 *  게임 월드 병렬화 실험 코드 — 다 실험 — Thread 수 확장성 비교
 * ============================================================
 *
 *  공통 기준:
 ...
 *
 *  빌드:
 *    gcc -O2 -o da_thread_scaling_play_fixed da_thread_scaling_play_fixed.c -lm -lpthread
 * ============================================================
 */
...
/* ============================================================
 * 섹션 10: [PLAY] Parent + Render Thread K개 파이프라인
 *
 *   tick n:
 *     PARENT: 물리/AI → 명령서 생성
 *     THREAD×K: tick n-1 렌더링, DRAWCALL_COUNT를 K등분
 *
 *   명령서 버퍼는 2개를 사용한다.
...
static void play_threaded_pipeline_simulation(const TerrainMap *tmap,
                                              ...
                                              int worker_count)
{
    printf("[PLAY] Parent + Render Thread %d개 파이프라인 시작...\n", worker_count);
    ...
    pthread_t render_threads[8];
    PlayRenderThreadArg args[8];
    memset(args, 0, sizeof(args));

    double create_start = now_ms();
    for (int i = 0; i < worker_count; i++) {
        int start_draw = (DRAWCALL_COUNT * i) / worker_count;
        int end_draw = (DRAWCALL_COUNT * (i + 1)) / worker_count;
        args[i].tmap = tmap;
        args[i].cmd_buffers = cmd_buffers;
        args[i].frame_acc = frame_acc;
        args[i].worker_id = i;
        args[i].worker_count = worker_count;
        args[i].pin_cpu = role_cpu(i + 1);
        args[i].start_draw = start_draw;
        args[i].end_draw = end_draw;
        args[i].mutex = &mutex;
        args[i].cond_job = &cond_job;
        args[i].cond_done = &cond_done;
        args[i].stop = &stop;
        args[i].job_generation = &job_generation;
        args[i].job_in_progress = &job_in_progress;
        args[i].job_tick = &job_tick;
        args[i].job_buffer = &job_buffer;
        args[i].completed_workers = &completed_workers;
        args[i].rendered_ticks = &rendered_ticks;
        args[i].buffer_busy = buffer_busy;
        args[i].render_ms_sum = &render_ms_sum;
        args[i].pixel_sum = &pixel_sum;
        int rc = pthread_create(&render_threads[i], NULL, play_render_thread_main, &args[i]);
        if (rc != 0) {
            ...
            for (int j = 0; j < i; j++) pthread_join(render_threads[j], NULL);
            free(cmd_buffers[0]);
            free(cmd_buffers[1]);
            free(frame_acc);
            return;
    ...
    for (int tick = 0; tick < PLAY_TICKS; tick++) {
        ...
        if ((tick + 1) % progress_step == 0) {
            printf("[PLAY] tick %3d/%d | Parent 누적 %.1fms | Render Thread×%d 진행 중\n",
                   tick + 1, PLAY_TICKS, t_physics + t_cmd, worker_count);
        }
    }
    ...
    for (int i = 0; i < worker_count; i++) pthread_join(render_threads[i], NULL);

    printf("[PLAY] 완료\n");
    printf("[PLAY] 물리/AI:       %8.2f ms (Parent)\n", t_physics);
    printf("[PLAY] 명령서 생성:   %8.2f ms (Parent)\n", t_cmd);
    printf("[PLAY] GPU 렌더링:    %8.2f ms (Render Thread×%d 실제 계산 누적)\n", render_ms_sum, worker_count);
...
/* ============================================================
 * 다 실험 — Thread 수 1→2→4→8 확장성 비교
 * - Thread K개: noise_compute_chunk()를 chunk 단위로 분할 수행
 * - Parent: Thread K개와 동시에 조도 계산 수행
 * - noise 완료 후 join으로 merge 시작 보장
 * - merge → blur → erosion → histogram/integrity 순서는 Baseline 기준 유지
 * ============================================================ */
...
static void *noise_thread_main(void *arg)
{
    NoiseThreadArg *a = (NoiseThreadArg *)arg;
    pin_current_thread_to_cpu(a->pin_cpu);
    double t = now_ms();

    for (int ci = a->start_chunk; ci < a->end_chunk; ci++) {
        noise_compute_chunk(&a->tmap->chunks[ci], &a->tmap->params);
    }

    a->finish_time = now_ms();
    a->noise_ms = a->finish_time - t;
    printf("[THREAD %d] chunk %d~%d 완료: %.2f ms\n",
           a->tid, a->start_chunk, a->end_chunk - 1, a->noise_ms);
    return NULL;
}

static void print_da_report(const PerfMetrics *m,
                            int worker_count,
                            int active_n_setting,
                            double baseline_ms,
                            double noise_wall_ms,
                            double noise_seq_ms,
                            double setting_wall_ms,
                            double transition_ms,
                            double play_wall_ms,
                            double total_wall_ms,
                            double pthread_create_total_ms,
                            double join_wait_min_ms,
                            double join_wait_max_ms,
                            double join_wait_avg_ms,
...
{
    const int n_play = worker_count + 1;
    double setting_parallel_seq = noise_seq_ms + m->parent_lighting_ms;
    double setting_parallel_wall = noise_wall_ms > m->parent_lighting_ms ? noise_wall_ms : m->parent_lighting_ms;
    double setting_parallel_speedup = setting_parallel_wall > 0.0 ? setting_parallel_seq / setting_parallel_wall : 0.0;
    double setting_seq = noise_seq_ms + m->thread_merge_ms +
                         m->thread_blur_ms + m->thread_erosion_ms +
                         m->thread_integrity_ms + m->parent_lighting_ms +
    ...
    double p_eff = calc_p_effective(total_speedup, active_n_setting);
    double s_theory = calc_amdahl(p_eff, active_n_setting);
    double cpu_util = total_wall_ms > 0.0 ?
        (total_usage->user_ms + total_usage->sys_ms) / total_wall_ms * 100.0 : 0.0;

    printf("\n===========================================================\n");
    printf("=== 게임 월드 병렬 실험 — 다 ===\n");
    printf("시드: 0x%08X (고정) | 맵: %dx%d | 청크: %d개\n", FIXED_SEED, MAP_WIDTH, MAP_HEIGHT, TOTAL_CHUNKS);
    printf("\n[CORE] 시스템 코어 수: %d개\n", get_core_count());
    printf("[PIN] 적용 시도: PARENT → cpu0 / WORKER·RENDER → cpu1~cpuK (코어 부족 시 worker 일부 공유)\n");

    printf("\n===========================================================\n");
    printf("[실험 구성]\n");
    printf("===========================================================\n");
    printf("  Worker 종류: Thread\n");
    printf("  Worker 수 K: %d\n", worker_count);
    printf("  Parent 동시 계산 여부: 계산\n");
    printf("  active core 수 N_setting: %d\n", active_n_setting);
    printf("  active core 수 N_play:    %d\n", n_play);
    printf("  N 계산 근거: 세팅 noise 구간에서 Thread %d개가 chunk 계산, Parent 1개가 조도 계산을 동시에 수행. 플레이 중 Parent는 현재 tick 물리/명령서, Thread K개는 이전 tick 렌더링을 분할 수행.\n", worker_count);

    printf("\n===========================================================\n");
    printf("[세팅 단계]\n");
    printf("===========================================================\n");
    printf("  [WORKER×%d] 노이즈 연산 Wall: %10.2f ms  (완료 후 merge 가능해야 함)\n", worker_count, noise_wall_ms);
    printf("  [WORKER×%d] 노이즈 순차 환산: %10.2f ms\n", worker_count, noise_seq_ms);
    ...
    printf("  세팅 병렬 구간 순차 환산:   %10.2f ms  (= 노이즈 순차 환산 + 조도)\n", setting_parallel_seq);
    printf("  세팅 병렬 구간 Wall:        %10.2f ms  (= max(노이즈 Wall, 조도))\n", setting_parallel_wall);
    printf("  세팅 병렬 구간 Speedup:     %10.2fx\n", setting_parallel_speedup);
    printf("  세팅 단계 Wall Time:        %10.2f ms\n", setting_wall_ms);
    printf("  세팅 전체 순차 환산:        %10.2f ms  (= 병렬 구간 + merge/blur/erosion/hist + AI + 파티클)\n", setting_seq);
    printf("  세팅 Speedup:               %10.2fx\n", setting_speedup);
    printf("  세팅 Efficiency:            %10.1f%%  (= Speedup / N_setting)\n", calc_efficiency(setting_speedup, active_n_setting));

    printf("\n[프로세스/스레드 생성]\n");
    printf("  fork() × K:                 N/A\n");
    printf("  pthread_create() × K:       %10.3f ms\n", pthread_create_total_ms);
    printf("  생성 총 오버헤드:            %10.3f ms\n", pthread_create_total_ms);

    printf("\n===========================================================\n");
    printf("[세팅→플레이 전환]\n");
    printf("===========================================================\n");
    printf("  추가 pthread_create() × K:  %10.3f ms  (play render threads)\n", m->play_thread_create_ms);
    ...
    printf("  [WORKER×%d] GPU 렌더링:    %10.2f ms  (DRAWCALL_COUNT K등분, 이전 tick 렌더링)\n", worker_count, m->play_render_ms);
    ...
    printf("\n[동기화/Barrier 대기 시간]\n");
    printf("  선행 동기화(noise 완료→merge 시작): min %.2f ms | max %.2f ms | avg %.2f ms\n",
           join_wait_min_ms, join_wait_max_ms, join_wait_avg_ms);
    printf("  BARRIER 1(merge 완료→blur 시작):    min 0.00 ms | max 0.00 ms | avg 0.00 ms\n");
    printf("  BARRIER 2(blur 완료→erosion 시작):  min 0.00 ms | max 0.00 ms | avg 0.00 ms\n");
    printf("  BARRIER 3(erosion 완료→hist/integrity): min 0.00 ms | max 0.00 ms | avg 0.00 ms\n");

    printf("\n===========================================================\n");
    printf("  총 Wall Time:               %10.2f ms\n", total_wall_ms);
    if (baseline_ms > 0.0) {
        printf("  총 Speedup:                 %10.2fx  (기준: Baseline %.2f ms)\n", total_speedup, baseline_ms);
        printf("  총 Efficiency:              %10.1f%%  (= Speedup / 대표 N_setting)\n", calc_efficiency(total_speedup, active_n_setting));
    } else {
    ...
    for (int wi = 0; wi < worker_count; wi++) {
        int cpu = role_cpu(wi + 1);
        printf("    cpu%-2d (WORKER%-2d):    ", cpu, wi + 1);
        print_percent_or_na(cpu_usage_between(cpu_setting_start, cpu_setting_end, cpu));
        printf("        cpu%-2d (RENDER%-2d):    ", cpu, wi + 1);
        print_percent_or_na(cpu_usage_between(cpu_play_start, cpu_play_end, cpu));
        printf("\n");
    }
    printf("    나머지:               ");
    print_percent_or_na(cpu_usage_avg_range(cpu_setting_start, cpu_setting_end, 1 + worker_count, 128));
    printf("        나머지:               ");
    print_percent_or_na(cpu_usage_avg_range(cpu_play_start, cpu_play_end, 1 + worker_count, 128));
    printf("\n");
    printf("===========================================================\n");
    printf("[컨텍스트 스위칭]\n");
...
static int run_thread_scaling_once(int worker_count, double baseline_ms)
{
    if (!(worker_count == 1 || worker_count == 2 || worker_count == 4 || worker_count == 8)) {
        fprintf(stderr, "Worker 수는 1, 2, 4, 8 중 하나여야 합니다.\n");
        return 1;
    }
    ...
    pthread_t threads[8];
    NoiseThreadArg args[8];
    memset(args, 0, sizeof(args));

    printf("\n===========================================================\n");
    printf("[다 실험 시작] Thread K = %d\n", worker_count);
    printf("===========================================================\n");

    double create_start = now_ms();
    for (int i = 0; i < worker_count; i++) {
        int start = (TOTAL_CHUNKS * i) / worker_count;
        int end   = (TOTAL_CHUNKS * (i + 1)) / worker_count;
        args[i].tmap = tmap;
        args[i].tid = i;
        args[i].start_chunk = start;
        args[i].end_chunk = end;
        args[i].noise_ms = 0.0;
        args[i].finish_time = 0.0;
        args[i].pin_cpu = role_cpu(i + 1);
        int rc = pthread_create(&threads[i], NULL, noise_thread_main, &args[i]);
        if (rc != 0) {
    ...
    double pthread_create_total_ms = now_ms() - create_start;
    ...
    double merge_ready_time = now_ms();
    double join_wait_min_ms = 1e100;
    double join_wait_max_ms = 0.0;
    double join_wait_sum_ms = 0.0;
    for (int i = 0; i < worker_count; i++) {
        double wait_i = args[i].finish_time > 0.0 ? merge_ready_time - args[i].finish_time : 0.0;
        if (wait_i < 0.0) wait_i = 0.0;
        if (wait_i < join_wait_min_ms) join_wait_min_ms = wait_i;
        if (wait_i > join_wait_max_ms) join_wait_max_ms = wait_i;
        join_wait_sum_ms += wait_i;
    }
    double join_wait_avg_ms = join_wait_sum_ms / worker_count;

    double max_noise_ms = 0.0;
    double sum_noise_ms = 0.0;
    for (int i = 0; i < worker_count; i++) {
        if (args[i].noise_ms > max_noise_ms) max_noise_ms = args[i].noise_ms;
        sum_noise_ms += args[i].noise_ms;
    }
    m.thread_noise_ms = max_noise_ms;

    printf("[PARENT] 모든 Thread noise 완료 확인. merge 시작 가능.\n");
    ...
    play_threaded_pipeline_simulation(tmap, &m, worker_count);
...
int main(int argc, char **argv)
{
    ...
    printf("=== 게임 월드 병렬 실험 — 다: Thread 수 1→2→4→8 확장성 비교 ===\n");
    printf("Baseline Wall Time 인자: %.2f ms%s\n", baseline_ms, baseline_ms > 0.0 ? "" : " (미입력)");
    printf("컴파일 예: gcc -O2 -o da_thread_scaling_play_fixed da_thread_scaling_play_fixed.c -lm -lpthread\n");
    printf("실행 예1: ./da_thread_scaling_play_fixed 310833\n");
    printf("실행 예2: ./da_thread_scaling_play_fixed 310833 4\n\n");

    if (argc >= 3) {
        int k = atoi(argv[2]);
        return run_thread_scaling_once(k, baseline_ms);
    }

    int ks[] = {1, 2, 4, 8};
    for (int i = 0; i < 4; i++) {
        int rc = run_thread_scaling_once(ks[i], baseline_ms);
        if (rc != 0) return rc;
    }
    return 0;
}

...
static void *thread_worker(void *arg) {
    ...
    /* BARRIER 1: 모든 스레드의 노이즈 + 즉시 병합 완료 */
    pthread_barrier_wait(bar);
    /*
    pthread_mutex_lock(&custom_b1_mutex);
    custom_b1_count++;
    if (custom_b1_count == g_num_threads) {
        pthread_cond_broadcast(&custom_b1_cond);
    } else {
        while (custom_b1_count < g_num_threads) {
            pthread_cond_wait(&custom_b1_cond, &custom_b1_mutex);
        }
    }
    pthread_mutex_unlock(&custom_b1_mutex);
    */
    // sem_wait(&sem_b1_mutex);
    // sem_b1_count++;
    // if (sem_b1_count == g_num_threads) {
    //     sem_post(&sem_b1_turnstile); /* 마지막 스레드가 첫 번째 대기자를 깨움 */
    // }
    // sem_post(&sem_b1_mutex);
    // sem_wait(&sem_b1_turnstile); /* 대기 (목표 카운트 도달 시 통과) */
    // sem_post(&sem_b1_turnstile); /* 통과하면서 다음 대기자를 연쇄적으로 깨움 */
    ...
    /* BARRIER 2: terrain_blur 완료 후 모든 스레드가 침식 단계로 이동 */
    // pthread_barrier_wait(bar);
    /* //뮤텍스
    pthread_mutex_lock(&custom_b2_mutex);
    custom_b2_count++;
    if (custom_b2_count == g_num_threads) {
        pthread_cond_broadcast(&custom_b2_cond);
    } else {
        while (custom_b2_count < g_num_threads) {
            pthread_cond_wait(&custom_b2_cond, &custom_b2_mutex);
        }
    }
    pthread_mutex_unlock(&custom_b2_mutex);
    *///세마포어 
    // sem_wait(&sem_b2_mutex);
    // sem_b2_count++;
    // if (sem_b2_count == g_num_threads) {
    //     sem_post(&sem_b2_turnstile);
    // }
    // sem_post(&sem_b2_mutex);
    // sem_wait(&sem_b2_turnstile);
    // sem_post(&sem_b2_turnstile);
    ...
    pthread_barrier_wait(bar);
    /*
    pthread_mutex_lock(&custom_b3_mutex);
    custom_b3_count++;
    if (custom_b3_count == g_num_threads) {
        pthread_cond_broadcast(&custom_b3_cond);
    } else {
        while (custom_b3_count < g_num_threads) {
            pthread_cond_wait(&custom_b3_cond, &custom_b3_mutex);
        }
    }
    pthread_mutex_unlock(&custom_b3_mutex);
    */
    // sem_wait(&sem_b3_mutex);
    // sem_b3_count++;
    // if (sem_b3_count == g_num_threads) {
    //     sem_post(&sem_b3_turnstile);
    // }
    // sem_post(&sem_b3_mutex);
    // sem_wait(&sem_b3_turnstile);
    // sem_post(&sem_b3_turnstile);

...
/*
 * ============================================================
 *  게임 월드 병렬화 실험 코드 — 나 실험 — Parent + 단일 Thread
 * ============================================================
 *
 *  공통 기준:
                            ...
                            const CpuSnapshot *cpu_play_end)
{
    ...
    double setting_parallel_wall = m->thread_total_ms > m->parent_lighting_ms ?
                                   m->thread_total_ms : m->parent_lighting_ms;
    double setting_parallel_speedup = setting_parallel_wall > 0.0 ?
                                      setting_parallel_seq / setting_parallel_wall : 0.0;
    double setting_seq = setting_parallel_seq + m->child1_ai_ms + m->child2_particle_ms;

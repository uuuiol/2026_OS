/*
 * ============================================================
 *  da_child4.c  —  다 실험: PARENT1 + CHILD4 병렬
 *
 *  세팅 단계:
...
int main(void){
    ...
    /* Speedup / Efficiency
       ...
       통합 N = 3 */
    int N_setting = 5;  /* Parent+Child4 */  /* Parent(조도+AI+파티클) + Child1 + Child2 */
    int N_play    = 5;  /* Parent(물리+명령서) + Child1(렌더) + Child2(렌더) */
    ...
    printf("  active core N (세팅): %d  ← Parent 계산+Child4\n", N_setting);
    printf("  active core N (플레이): %d  ← Parent+Child4 렌더\n", N_play);
    ...
    printf("  [CHILD4] noise 청크192~255:  %.2f ms\n", shm->child_elapsed_ms[3]);
    ...
    printf("  [CHILD4] GPU 렌더2048~2047:  %.2f ms  (틱평균 %.2f ms)\n",

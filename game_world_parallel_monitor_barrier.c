 ...
 *  이 파일은 Barrier 1, 2, 3에 monitor 방식 동기화를 적용한 버전 
 * ============================================================
 *
 *  시스템 개요:
...
/* ============================================================
 * 커스텀 BARRIER 구현 2: 모니터 방식
 * - 모니터는 공유 상태(count)와 동기화 연산(wait)을 구조체/함수 내부에 묶은 형태다.
 * - 내부적으로는 mutex/condition을 사용하지만, thread_worker에서는 monitor_barrier_wait()만 호출한다.
 * - 이 프로그램에서는 Barrier 1, 2, 3이 각각 1회만 사용되므로 one-shot monitor barrier로 충분하다.
 * ============================================================ */
typedef struct {
...
static MonitorBarrier mon_b1 = { PTHREAD_MUTEX_INITIALIZER, PTHREAD_COND_INITIALIZER, 0 };
static MonitorBarrier mon_b2 = { PTHREAD_MUTEX_INITIALIZER, PTHREAD_COND_INITIALIZER, 0 };
static MonitorBarrier mon_b3 = { PTHREAD_MUTEX_INITIALIZER, PTHREAD_COND_INITIALIZER, 0 };

static void monitor_barrier_wait(MonitorBarrier *mon, int target_count)
{
    pthread_mutex_lock(&mon->mutex);
    mon->count++;

    if (mon->count == target_count) {
        pthread_cond_broadcast(&mon->cond);
    } else {
        while (mon->count < target_count) {
...
static void *thread_worker(void *arg) {
    ...
    a->elapsed_noise_ms = now_ms() - t;
        ...
        /* BARRIER 1: 모니터 방식 */
    monitor_barrier_wait(&mon_b1, g_num_threads);
...
/* 단계2: baseline과 동일한 경계 블러
    ...
    if (tid == 0) {
        ...
        /* BARRIER 2: 모니터 방식 */
    monitor_barrier_wait(&mon_b2, g_num_threads);
...
/* 단계3: 침식 (가상 구역 분할) */
    ...
    a->elapsed_erosion_ms = now_ms() - t;
        ...
        /* BARRIER 3: 모니터 방식 */
    monitor_barrier_wait(&mon_b3, g_num_threads);

 ...
 *
 *  빌드:
 *    gcc -O2 -o da_thread_thread6 da_thread_thread6.c -lm -lpthread
 * ============================================================
 */
...
/* 병렬화 시 참조 */
#define MAX_WORKERS          8   /* 허용 최대 Thread 수: 1, 2, 4, 6, 8 */
                                              ...
                                              int worker_count)
{
    ...
    pthread_t render_threads[MAX_WORKERS];
    PlayRenderThreadArg args[MAX_WORKERS];
...
/* ============================================================
 * 다 실험 — Thread 수 1→2→4→6→8 확장성 비교
...
static int run_thread_scaling_once(int worker_count, double baseline_ms)
{
    if (!(worker_count == 1 || worker_count == 2 || worker_count == 4 ||
          worker_count == 6 || worker_count == 8)) {
        fprintf(stderr, "Worker 수는 1, 2, 4, 6, 8 중 하나여야 합니다.\n");
        return 1;
    }
    ...
    NoiseThreadArg args[MAX_WORKERS];
    memset(args, 0, sizeof(args));
...
int main(int argc, char **argv)
{
    ...
    printf("=== 게임 월드 병렬 실험 — 다: Thread 수 1→2→4→6→8 확장성 비교 ===\n");
    printf("Baseline Wall Time 인자: %.2f ms%s\n", baseline_ms, baseline_ms > 0.0 ? "" : " (미입력)");
    printf("컴파일 예: gcc -O2 -o da_thread_thread6 da_thread_thread6.c -lm -lpthread\n");
    printf("실행 예1: ./da_thread_thread6 310833\n");
    printf("실행 예2: ./da_thread_thread6 310833 6\n\n");

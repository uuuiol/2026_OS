 ...
 *
 *  빌드:
 *    gcc -O2 -o game_world_parallel_v1 game_world_parallel_v1.c -lm -lpthread
 * ============================================================
 */
...
typedef struct PlayShared {
    ...
    int              ready_buf;     /* THREAD가 렌더할 버퍼 인덱스 (-1=없음) */
...
int main(int argc, char **argv) {
    ...
    /* 히스토그램 + 무결성 (스레드 전체 클램핑 완료 후 안전) */
    double t_post = now_ms();
    terrain_histogram(tmap);
    double t_integ = now_ms();
    m.integrity_fails     = terrain_integrity(tmap);
    m.thread_integrity_ms = now_ms() - t_integ;
    m.thread_merge_ms     = now_ms() - t_post - m.thread_integrity_ms;
    printf("[THREAD] 무결성 실패: %d\n\n", m.integrity_fails);

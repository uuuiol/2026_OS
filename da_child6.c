/*
 * ============================================================
 *  da_child6.c  —  다 실험: PARENT1 + CHILD6 병렬
 *
 *  세팅 단계:
...
int main(void){
    ...
    /* Speedup / Efficiency
       ...
       통합 N = 3 */
    int N_setting = 7;  /* Parent+Child6 */  /* Parent(조도+AI+파티클) + Child1 + Child2 */
    int N_play    = 7;  /* Parent(물리+명령서) + Child1(렌더) + Child2(렌더) */
    ...
    printf("  active core N (세팅): %d  ← Parent 계산+Child6\n", N_setting);
    printf("  active core N (플레이): %d  ← Parent+Child6 렌더\n", N_play);
    printf("  N 근거: Parent가 조도/AI/파티클 실제 계산 참여 → N에 포함\n");
    printf("\n");

    printf("----- [CHILD×%d / TERRAIN] 세팅 지형 연산 -----\n", NUM_CHILDREN);
    printf("  [CHILD1] noise 청크  0~63:   %.2f ms\n", shm->child_elapsed_ms[0]);
    printf("  [CHILD2] noise 청크 64~127:  %.2f ms\n", shm->child_elapsed_ms[1]);
    printf("  [CHILD3] noise 청크128~191:  %.2f ms\n", shm->child_elapsed_ms[2]);
    printf("  [CHILD4] noise 청크192~213:  %.2f ms\n", shm->child_elapsed_ms[3]);
    printf("  [CHILD5] noise 청크214~234:  %.2f ms\n", shm->child_elapsed_ms[4]);
    printf("  [CHILD6] noise 청크235~255:  %.2f ms\n", shm->child_elapsed_ms[5]);
    ...
    printf("  [CHILD2] GPU 렌더1024~1535:  %.2f ms  (틱평균 %.2f ms)\n",
           shm->child_render_ms[1], shm->child_render_ms[1]/PLAY_TICKS);
    printf("  [CHILD3] GPU 렌더1536~2047:  %.2f ms  (틱평균 %.2f ms)\n",
           shm->child_render_ms[2], shm->child_render_ms[2]/PLAY_TICKS);
    printf("  [CHILD4] GPU 렌더:           %.2f ms  (틱평균 %.2f ms)\n",
           shm->child_render_ms[3], shm->child_render_ms[3]/PLAY_TICKS);
    printf("  [CHILD5] GPU 렌더:           %.2f ms  (틱평균 %.2f ms)\n",
           shm->child_render_ms[4], shm->child_render_ms[4]/PLAY_TICKS);
    printf("  [CHILD6] GPU 렌더:           %.2f ms  (틱평균 %.2f ms)\n",
           shm->child_render_ms[5], shm->child_render_ms[5]/PLAY_TICKS);
    ...
    for (int i=0;i<nc;i++){
        ...
        else if(i==3){slbl="(CHILD3)";plbl="(CHILD3-렌더)";}
        else if(i==4){slbl="(CHILD4)";plbl="(CHILD4-렌더)";}
        else if(i==5){slbl="(CHILD5)";plbl="(CHILD5-렌더)";}
        else if(i==6){slbl="(CHILD6)";plbl="(CHILD6-렌더)";}

 ...
 *  이 파일은 Barrier 1, 2, 3에 mutex 방식 동기화를 적용한 버전 
 * ============================================================
 *
 *  시스템 개요:
...
/* ============================================================
 * 커스텀 BARRIER 구현 1: 뮤텍스 + 조건변수 방식
 * - count를 mutex로 보호한다.
 * - 마지막 스레드가 도착하면 condition broadcast로 모두 깨운다.
 * - 이 프로그램에서는 Barrier 1, 2, 3이 각각 1회만 사용되므로 one-shot barrier로 충분하다.
 * ============================================================ */
static void mutex_barrier_wait(pthread_mutex_t *mutex,
                               pthread_cond_t *cond,
                               int *count,
                               int target_count)
{
...
static void *thread_worker(void *arg) {
    ...
    a->elapsed_noise_ms = now_ms() - t;
        ...
        /* BARRIER 1: 뮤텍스 + 조건변수 방식 */
    mutex_barrier_wait(&custom_b1_mutex, &custom_b1_cond, &custom_b1_count, g_num_threads);
...
/* 단계2: baseline과 동일한 경계 블러
    ...
    if (tid == 0) {
        ...
        /* BARRIER 2: 뮤텍스 + 조건변수 방식 */
    mutex_barrier_wait(&custom_b2_mutex, &custom_b2_cond, &custom_b2_count, g_num_threads);
...
/* 단계3: 침식 (가상 구역 분할) */
    ...
    a->elapsed_erosion_ms = now_ms() - t;
        ...
        /* BARRIER 3: 뮤텍스 + 조건변수 방식 */
    mutex_barrier_wait(&custom_b3_mutex, &custom_b3_cond, &custom_b3_count, g_num_threads);
